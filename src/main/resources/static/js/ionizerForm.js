...
$(document).ready(function () {
	...
	} else{// Now page can load safely
			// Proceed with lineStationSheetMappingId
			loadIonizerUpperFormBody(lineStationSheetMappingId);
			createIonizerFormBody(lineStationSheetMappingId);
...
function checkModelVariant(mappingId) {
    $.ajax({
        type: "GET",
        url: "/WebApplication/Controllers/ionizer/checkModelVariant/" + mappingId,
        ...
        error: function(xhr) {
            alert("Cannot load SOP because Model or Variant is missing.\n\n" +
				  ...
				  $('#submitIonizerSheet').prop("disabled", true);

				  $('#submitIonizerSheet').css({
				      pointerEvents: "none",
				      opacity: 0.5
				  });
				  
				  $("#ionizercheckformBody").html("<div class='text-danger'>Either Model or Variant is not found.</div>");
        }
    });
}
...
function loadIonizerUpperFormBody(lineStationSheetMappingId) {
    $("#upper_ionizer_form_body").empty();
    ...
    $.ajax({
        ...
        success: function(machine) {
            ...
            `;
            $("#upper_ionizer_form_body").html(html);
        },
        error: function() {
            $("#upper_ionizer_form_body").html("<div class='text-danger'>Machine data not found.</div>");
        }
    });
}


function createIonizerFormBody(lineStationSheetMappingId) {
	$("#ionizercheckformBody").empty();

	$.ajax({ 	// written in IonizerFormAjaxController
		url: `/WebApplication/Controllers/ionizer/machineparameters/${lineStationSheetMappingId}?resumeMode=${resumeMode}`,
		method: "GET",
		headers: {
		...
		success: function(data) {
			...
			data.forEach((item, index) => {
				...
				const row = `
	                ...
	                `;
				$("#ionizercheckformBody").append(row);
			});

			if (resumeMode) {
			    $.ajax({
			        url: `/WebApplication/Controllers/ionizer/lastData/${lineStationSheetMappingId}`,
			        method: "GET",
			        headers: {
			            Authorization: `Bearer ${sessionStorage.getItem('token')}`
			        },
									...
									if (sd.inputData) {
						                 try {
						                     ...
						                     Object.entries(parsed).forEach(([key, val]) => {
						                         // exactly same naming as in createIonizerFormBody
		...
		error: function(err) {
			...
			$("#ionizercheckformBody").html("<div class='text-danger'>Failed to load data</div>");
		}
	});
}
...
$('#submitIonizerSheet').click(function() {
	...
	if ($(this).data("submitted")) { // will run it for just only in frontend so this not work when page refreshed.
	     alert("This ionizer check sheet is already filled.");
	     return;
	 }


	console.log("Submitting for machine ID:", currentMachineId);
	const shift = $("#upper_ionizer_form_body input[name='shift']").val();
	const logicalShiftDateTime= $('#backendLogicalDateTimeText').text().trim();
	
	console.log("shift", shift);
	$('#ionizercheckformBody .row').each(function(index) {
		
		// taking data from hidden input created in form createIonizerFormBody()
		...
		console.log("linkId", linkId);
		 if (!linkId) {
			...
			$('#submitIonizerSheet').prop('disabled', false).text("Submit Ionizer Sheet")
			.css({
			    pointerEvents: "auto",
			    opacity: 1
			});
...
/*		if (!doneDate) {
	...
	let url = resumeMode
	  ? `/WebApplication/Controllers/ionizer/save-machine-user-input/resume/${lineStationSheetMappingId}`
	  : `/WebApplication/Controllers/ionizer/save-machine-user-input/${lineStationSheetMappingId}`;
	...
	$.ajax({
		...
		$('#submitIonizerSheet').data("submitted", true);
				
		// 		✔ Prevents accidental edits
		// ✔ Matches backend finalization
		$('#ionizercheckformBody input, #ionizercheckformBody select')
		    .prop('disabled', true);
...
function resetForm() {
	...
	$('#ionizercheckformBody input[type="text"], #ionizercheckformBody input[type="date"], #ionizercheckformBody input[type="number"], #ionizercheckformBody input[type="hidden"]').val('');

	// If you have select elements or other form elements, reset them too
	$('#ionizercheckformBody select').prop('selectedIndex', 0); // For select elements

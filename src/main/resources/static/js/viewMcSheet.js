...
$(document).ready(function() {
	...
	createMcSheetBody();
	loadMachineSheetUpperFormBody(lineStationSheetMappingId, 1);
	createMcSheetFooter();
	getMcSheetBodyData();
});
...
$(document).on('click', '#capture', function() {
	...
	// Clone print area
	var printArea = document.querySelector('.machineCheckSheetPrintArea').cloneNode(true);
...
$(document).on('click', '#approveDetails', function() {
	...
	var table = document.getElementById("mcDataTableBody");
	var rows = table.rows;
	var IdList = [];
	...
	$.ajax({
		type: 'post',
		url: '/WebApplication/Controllers/mcs/approveMcsData',
...
function appendApproveValueInForm() {
	...
	$("#mcs_approval_form_body").empty();
	showModal("detailBackdropButton");
	var headerHtml = $("#mcs_form_body").clone(true, true);
	//var bodyHtml = $("#mcs_form_body").html();

	$("#mcs_approval_form_body").append(headerHtml);
	//$("#mcs_approval_form_body").append(bodyHtml);
...
function loadMachineSheetUpperFormBody(lineStationSheetMappingId, countNo) {
	//$("#mcs_header_body").empty();
	...
	$.ajax({
		type: 'post',
		url: '/WebApplication/Controllers/mcs/getAllHeaderDataOfMcsSheet',
		data: JSON.stringify(formData),
		async: false,
		contentType: "application/json",
		headers: { Authorization: `Bearer ${sessionStorage.getItem('token')}` },
		success: function(res) {
			appendmachineSheetUppedFormBodyData(res);
...
function appendEmptyHeader() {
	...
	//$("#mcs_header_body").html(html);

	$("#mcDataTable").prepend(tableHeader);

}



function appendmachineSheetUppedFormBodyData(list) {
	...
	$("#mcDataTableHead").prepend(tableHeader);
...
function createMcSheetBody() {

	$.ajax({ 	// written in MachineCheckSheetFormAjaxController
		url: "/WebApplication/Controllers/mcs/ViewMcSheetparameters/" + lineStationSheetMappingId,
		method: "GET",
		async: false,
		headers: {
		...
		success: function(response) {
			...
			createMcSheetMiddleBody(response);

		},
		error: function(err) {
			console.error("Error fetching data", err, err.responseText);
			console.error("Error text", err.responseText);
			$("#mcs_form_body").html("<div class='text-danger'>Failed to load data</div>");
		}
	});
...
function createMcSheetMiddleBody(response) {
	...
	makeMcsTable(headerList, "mcs_form_body", "mcDataTable", "100%");
			...
			if (k == 1) {
				...
				for (var j = 0; j < whenList.length; j++) {
					...
					if (j < whenList.length - 1) {
						heading.setAttribute("class", whenList[j] + " py-2 px-2 border-bottom border-secondary");
						//div.setAttribute("style", "height:80px; width:80px;");
					} else {
						heading.setAttribute("class", whenList[j] + " py-2 px-2");
						//div.setAttribute("style", "height:80px; width:80px;");
					}
					
					heading.setAttribute("style", "font-size: 12px;");

					td.setAttribute("style", "min-width: 50px; max-width:50px;");
					td.append(heading);
				}
			}
			else if (k == 7) {
	...
	$("#mcDataTable").append(tableFooter);
...
function getMcSheetBodyData() {
	...
	$.ajax({
		type: 'post',
		url: '/WebApplication/Controllers/mcs/getAllDataOfMcSheetByIdAndDate',
		data: JSON.stringify(formData),
		async: false,
		contentType: "application/json",
		headers: { Authorization: `Bearer ${sessionStorage.getItem('token')}` },
		success: function(response) {
			appendDataInMcBody(response);
...
function appendDataInMcBody(list) {
	...
	for (let i = 0; i < list.length; i++) {
		...
		for (var j = 0; j < keysList.length; j++) {
			...
			//$("#input" + keysList[j] + date + list[i].field.rowId + j).val(value);

			/*console.log("input" + keysList[j] + date + list[i].field.rowId + j);
			
			*/
			//$("#input" + keysList[j] + date + list[i].field.rowId).attr("value", value);
...
function loadMCSUpperFormBody(lineStationSheetMappingId) {
	...
	$.ajax({
		...
		error: function() {
			$("#mcs_header_body").html("<div class='text-danger'>Machine data not found.</div>");
		}
	});
}

...
$(document).on('click', '#machineCheckSheetField', function() {

	$(".fromToDiv").css("display", "none");
	
	$("#offcanvasCloseButton").click();
	$("#masterHeading").text("MACHINE CHECK SHEET FIELD MASTER");

	$("#tableContainer").empty();
	$("#pagerContainer").empty();
	makePagerBody("pagerContainer");

/*	var headerList = ["S. No.", "Paramtype", "Parameter", "How", "Who",  "Vtype", "Passif", "Value1", "Value2", "Created by","Creation Date", "Date Modified"];
	var searchList = ["searchPtype", "searchParameter", "searchHow", "searchWho", "searchVtype", "searchPassif", "searchValue1", "searchValue2",  "", "", ""];
	var placeholderList = ["Paramtype", "Parameter", "How", "Who", "Vtype", "Passif", "Value1", "Value2", ""];*/
...
window.loadLikeMachineCheckSheetFieldData = loadLikeMachineCheckSheetFieldData;
function loadLikeMachineCheckSheetFieldData(page, pageSize) {
	...
	$.ajax({
		type: 'post',
		url: '/WebApplication/Controllers/getLikeMachineCheckSheetFieldData/' + page + '/' + pageSize,
	  ...
	  console.log(res);
			...
			insertMachineCheckSheetFieldInTable(res.content, "table1");
		},
...
window.machineCheckSheetFieldInputs =machineCheckSheetFieldInputs;
function machineCheckSheetFieldInputs(modalBodyId) {
...
window.insertMachineCheckSheetFieldInTable = insertMachineCheckSheetFieldInTable;
function insertMachineCheckSheetFieldInTable($item, tableId) {
	$("#" + tableId + "Body").empty();
	...
	$.each($item, function(index, value) {
		...
		var row = '<tr class="tableDataRows" title="Double click to select the row."><td data-column="columnId" class="width50">' + (sequenceNumber + 1) + '</td>'
			...
			+ '<td data-column="MCSfieldId" style="display:none;">' + (value.rowId == null ? '' : value.rowId) + '</td></tr>';
		$("#" + tableId).append(row);
		sequenceNumber++;
	});
}
...
window.addMachineCheckSheetField = addMachineCheckSheetField;
function addMachineCheckSheetField() {
	...
	$.ajax({
		type: 'POST',
		url: '/WebApplication/Controllers/insertMCCheckSheetField',
...
window.editMachineCheckSheetField = editMachineCheckSheetField;
function editMachineCheckSheetField() {
	...
	if (vtype === "number") {
		...
		const formData = new FormData();
		formData.append("rowid", MCSfieldId); // 🔴 Add this — assumed your edit uses `rowId`
		formData.append("ptype", ptype);
		...
		$.ajax({
			type: 'POST',
			url: '/WebApplication/Controllers/editMCCheckSheetField',
			data: formData, //  may also contain the input image file
			processData: false,
			contentType: false,
			headers: { Authorization: `Bearer ${sessionStorage.getItem('token')}` },
...
window.deleteMachineCheckSheetField = deleteMachineCheckSheetField;
function deleteMachineCheckSheetField() {

	if (deleteAllList.length >= 1) {

		$.ajax({
			type: 'post',
			url: '/WebApplication/Controllers/delete/deleteAllMachineCheckSheetField',
